  ...
  if (config.AUTH_MODE === 'local') {
    return async (request) => {
      ...
      const result = await config.db.query(
        'SELECT token_hash,csrf_token_hash,user_id,tenant_id,expires_at,last_seen_at,rotated_at,ip_hash,user_agent,risk_level,passkey_verified_at FROM admin_sessions WHERE token_hash=$1 AND expires_at>now() AND revoked_at IS NULL',
        [hashToken(token)]
      );
