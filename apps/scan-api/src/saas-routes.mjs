...
export function registerSaasRoutes(app, { db,config }) {
  ...
  app.get("/api/v1/documents/:id/objects",async(request)=>{
    ...
    const result=await db.query(`SELECT object_id,expected,object_snapshot,added_by,added_at FROM business_document_objects
  ...
  app.post("/api/v1/documents/:id/objects",async(request,reply)=>{
    requireCapability(request.principal,"documents:write");const documentId=uuid.parse(request.params.id),body=z.object({objectCode:z.string().trim().min(6).max(200).transform(value=>value.toUpperCase()),expected:z.boolean().default(true),auditReason:reason}).parse(request.body);
    const response=await command(db,request,"DOCUMENT_OBJECT_ADD",{documentId,...body},async client=>{
      ...
      const snapshot=object.rows[0],created=await client.query(`INSERT INTO business_document_objects(tenant_id,document_id,object_id,expected,object_snapshot,added_by)
        VALUES($1,$2,$3,$4,$5,$6) ON CONFLICT(tenant_id,document_id,object_id) DO UPDATE SET expected=EXCLUDED.expected,object_snapshot=EXCLUDED.object_snapshot,added_by=EXCLUDED.added_by,added_at=now() RETURNING *`,[request.principal.tenantId,documentId,snapshot.id,body.expected,snapshot,request.principal.id]);
