...
export function registerPasskeyRoutes(app,{db,config}){
  ...
  app.delete("/api/auth/sessions/:id",async(request,reply)=>{const id=z.string().uuid().parse(request.params.id);const removed=await db.transaction(async client=>{const result=await client.query("UPDATE admin_sessions SET revoked_at=now(),revoked_by=$1,revocation_reason='User revoked session' WHERE id=$2 AND user_id=$1 AND revoked_at IS NULL RETURNING id,tenant_id",[request.principal.id,id]);if(result.rowCount)await client.query(`INSERT INTO authentication_events(tenant_id,user_id,event_type,risk_level,actor_id,reason) VALUES($1,$2,'SESSION_REVOKED','LOW',$2,'User revoked session')`,[result.rows[0].tenant_id,request.principal.id]);return result;});if(!removed.rowCount)return reply.code(404).send({code:"NOT_FOUND",message:"Session not found"});return reply.code(204).send();});
  app.post("/api/auth/sessions/revoke-others",async(request)=>{const body=z.object({reason:z.string().trim().min(3).max(500).default("User revoked all other sessions")}).parse(request.body||{});const result=await db.transaction(async client=>{const changed=await client.query("UPDATE admin_sessions SET revoked_at=now(),revoked_by=$1,revocation_reason=$3 WHERE user_id=$1 AND token_hash<>$2 AND revoked_at IS NULL",[request.principal.id,request.authSession.token_hash,body.reason]);if(changed.rowCount)await client.query(`INSERT INTO authentication_events(tenant_id,user_id,event_type,risk_level,actor_id,reason,metadata) VALUES($1,$2,'SESSION_REVOKED','LOW',$2,$3,$4)`,[request.principal.tenantId,request.principal.id,body.reason,{count:changed.rowCount,scope:"OTHER_SESSIONS"}]);return changed;});return{revoked:result.rowCount};});
}
