...
function scan() {
  ...
  const assessment = evaluateCampaign(carton); pendingReceipt = { code, assessment }; out.className="scan-result result-success"; out.innerHTML=`<b>可靠码验证通过</b><p>${carton.sku} · 批次 ${carton.batch} · ${carton.units} 袋 · 属于发货单 <span class="code">${state.shipment.id}</span>。</p><p>${assessment.eligible ? `符合 ${assessment.campaign.id} ${assessment.campaign.version}，确认后冻结 ${assessment.amount} 积分。` : `可以收货，但本次不计奖：${assessment.reason}。`}</p><button class="primary" id="confirm-receipt">确认收货（1 箱 / ${carton.units} 袋）</button>`; $("#confirm-receipt").onclick = confirmReceipt; toast("已验证可靠码，请复核后确认收货");
}
function confirmReceipt() {
  const out = $("#scan-result"), pending = pendingReceipt;
  if (!pending) return toast("请重新验证可靠码后再确认收货");
  const carton = state.cartons[pending.code];
  if (!carton || carton.status === "已收货") { pendingReceipt = null; return toast("该箱状态已变化，请重新验证"); }
  const id = eventId(), assessment = createReceipt(pending.code, carton, id, "首次有效收货"); pendingReceipt = null; save(); renderDashboard(); renderLedger(); renderReceive(); updateChrome(); const result = $("#scan-result"); result.className="scan-result result-success"; result.innerHTML=`<b>收货确认成功 ${assessment.eligible ? tag("冻结中") : tag("不计奖")}</b><p>${carton.sku} · 批次 ${carton.batch} · 已确认 ${carton.units} 袋。</p><p>追溯事件 <span class="code">${id}</span>；${assessment.eligible ? `奖励 +${assessment.amount} 积分，冻结 ${assessment.campaign.hold} 天后进入结算。` : `本次不计奖：${assessment.reason}。`}</p>`; toast(assessment.eligible ? "已确认收货并创建冻结奖励" : "已确认收货，本次不计奖");
}
...
function renderRisk() {
  ...
  document.querySelectorAll("[data-risk]").forEach((b) => b.onclick = () => { const r=state.risks.find((x)=>x.id===Number(b.dataset.risk)), action=b.dataset.action, time=now(); if(!r || r.status!=="待处理") return; if(action==="approve"){r.status="已通过";r.decision=`风险核验通过；仅完成异常处置，待重新验证收货 · ${time}`;state.events.unshift({id:eventId("-APPROVED"),type:"风险核验通过",code:r.code,actor:"可靠码风控组",time,result:`关联 ${r.event}；未确认收货，不创建奖励分录`});}else if(action==="reject"){r.status="已拒绝";r.decision=`拒绝奖励，保留待核验事实 · ${time}`;state.events.unshift({id:eventId("-REJECTED"),type:"奖励核验拒绝",code:r.code,actor:"可靠码风控组",time,result:`关联 ${r.event}；不创建正向奖励分录`});}else{r.decision=`继续冻结，等待补充证据 · ${time}`;}r.time=time;save();renderAll();toast(action==="approve"?"风险已核验通过；请重新验证并确认收货":action==="reject"?"已拒绝本次奖励申请，原始事实已保留":"风险继续待处理，等待补充证据"); });
}
