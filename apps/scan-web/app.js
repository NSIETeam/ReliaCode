...
function renderCodes() {
  ...
  const batchRows=state.codeBatches.map((batch)=>`<div class="code-batch-row"><div><b class="code">${escapeHtml(batch.id)}</b><small>${escapeHtml(localTime(batch.createdAt))}</small></div><span>${escapeHtml(product(batch.productId)?.name||"产品已删除")}</span><span>${escapeHtml(batch.level)} / ${batch.quantity.toLocaleString()}</span><span class="code">${escapeHtml(batch.firstCode)}<br/>${escapeHtml(batch.lastCode)}</span>${tag(batch.status)}<div class="batch-actions"><button class="row-action" data-labels="${batch.id}">${icon("scan")}二维码标签</button><button class="row-action" data-download="${batch.id}">${icon("download")}CSV</button></div></div>`).join("");
...
function addProduct(event) { event.preventDefault();const data=new FormData(event.currentTarget);const sku=String(data.get("sku")).trim(),gtin=String(data.get("gtin")).trim();if(state.products.some((item)=>item.sku.toLowerCase()===sku.toLowerCase()))return toast("SKU 已存在");if(gtin&&!validGtin(gtin))return toast("GTIN 必须是有效的 8、12、13 或 14 位编码（含校验位）");state.products.push({id:uuid(),name:String(data.get("name")).trim(),sku,gtin,createdAt:timestamp()});save();renderAll();go("codes");toast("产品已保存"); }
