...
const EMPTY_STATE = Object.freeze({
  ...
  products: [], codeBatches: [], documents: [], devices: [], objects: {}, events: [], campaigns: [], ledger: [], risks: [], agentRuns: [], gs1Settings:null
});
const svgPaths = {
...
let operationalDeviceCredential = { id:"", token:"" };
let agentReturnFocus = null;
let cameraStream = null;
let cameraFrame = 0;
let cameraBusy = false;
const initialUrl=new URL(location.href);
let pendingDeepLink = initialUrl.searchParams.get("verify");
let resetToken=initialUrl.searchParams.get("resetToken"),verifyEmailToken=initialUrl.searchParams.get("verifyEmailToken");
...
async function serverRequest(path, options={}) {
  const {_stepUpAttempted=false,...requestOptions}=options;
  const response = await fetch(apiUrl(path), { credentials:"include", ...requestOptions, headers:{ Accept:"application/json", ...(requestOptions.body?{"Content-Type":"application/json"}:{}), ...(requestOptions.headers||{}) } });
  const rotatedCsrf=response.headers.get("x-csrf-token");if(rotatedCsrf)csrfToken=rotatedCsrf;
  const body = await response.json().catch(()=>({}));
  if (!response.ok) {
    if(response.status===428&&body.code==='PASSKEY_STEP_UP_REQUIRED'&&!_stepUpAttempted){await verifyPasskeyStepUp();return serverRequest(path,{...requestOptions,_stepUpAttempted:true});}
    const error = new Error(body.message || ("Request failed (" + response.status + ")")); error.status=response.status; error.body=body; throw error;
  }
  return body;
}
...
async function verifyPasskeyStepUp(){if(!window.PublicKeyCredential)throw new Error('此操作需要 Passkey 二次验证，但当前浏览器不支持');const options=await serverRequest('/api/auth/passkeys/step-up/options',{method:'POST',headers:{'X-CSRF-Token':csrfToken},body:'{}',_stepUpAttempted:true});const credential=await navigator.credentials.get({publicKey:authenticationOptions(options)});if(!credential)throw new Error('Passkey 二次验证已取消');return serverRequest('/api/auth/passkeys/step-up/verify',{method:'POST',headers:{'X-CSRF-Token':csrfToken},body:JSON.stringify({challenge:options.challenge,response:credentialJson(credential)}),_stepUpAttempted:true});}
function securityDialog(title){let overlay=$('#security-dialog');if(overlay)overlay.remove();overlay=document.createElement('div');overlay.id='security-dialog';overlay.className='onboarding';overlay.innerHTML=`<section class="onboarding-card auth-card" role="dialog" aria-modal="true" aria-labelledby="security-dialog-title"><button class="onboarding-close" type="button" aria-label="关闭">${icon('close')}</button><h1 id="security-dialog-title">${escapeHtml(title)}</h1><div id="security-dialog-body"></div></section>`;document.body.append(overlay);overlay.querySelector('.onboarding-close').onclick=()=>overlay.remove();return overlay.querySelector('#security-dialog-body');}
async function managePasskeys(){try{const result=await serverRequest('/api/auth/passkeys'),body=securityDialog('Passkey 设备');body.innerHTML=result.items.length?`<p>撤销丢失或不再使用的设备。管理员账号撤销后必须至少保留两个 Passkey。</p>${result.items.map(item=>`<div class="movement-row"><div><b>${escapeHtml(item.name)}</b><small>${escapeHtml(item.device_type||'Passkey')} · ${escapeHtml(item.last_used_at?localTime(item.last_used_at):'尚未使用')}</small></div><button type="button" class="row-action" data-revoke-passkey="${escapeHtml(item.id)}">撤销</button></div>`).join('')}`:emptyState('尚未注册 Passkey','请先添加至少两个独立设备。');body.querySelectorAll('[data-revoke-passkey]').forEach(button=>button.onclick=async()=>{if(!confirm('确定撤销这个 Passkey？该设备将不能再用于登录。'))return;try{await serverRequest('/api/auth/passkeys/'+encodeURIComponent(button.dataset.revokePasskey),{method:'DELETE',headers:{'X-CSRF-Token':csrfToken,'Idempotency-Key':uuid()}});toast('Passkey 已撤销');await managePasskeys();}catch(error){toast(error.message);}});}catch(error){toast(error.message);}}
async function manageSessions(){try{const result=await serverRequest('/api/auth/sessions'),body=securityDialog('登录设备'),active=result.items.filter(item=>!item.revoked_at);body.innerHTML=`<p>发现陌生设备时立即将其退出。IP 地址不会在此处显示或存储为明文。</p><button id="revoke-other-sessions" class="secondary" type="button" ${active.filter(item=>!item.current).length?'':'disabled'}>退出其他所有设备</button>${result.items.map(item=>`<div class="movement-row"><div><b>${escapeHtml(item.current?'当前设备':(item.user_agent||'未知设备'))}</b><small>${escapeHtml(item.auth_method||'UNKNOWN')} · ${escapeHtml(item.risk_level||'LOW')} 风险 · 最近活动 ${escapeHtml(localTime(item.last_seen_at))}${item.revoked_at?' · 已退出':''}</small></div>${!item.current&&!item.revoked_at?`<button type="button" class="row-action" data-revoke-session="${escapeHtml(item.id)}">退出</button>`:''}</div>`).join('')||emptyState('没有会话记录','登录后会在这里显示设备。')}`;body.querySelectorAll('[data-revoke-session]').forEach(button=>button.onclick=async()=>{if(!confirm('确定让该设备立即退出？'))return;try{await serverRequest('/api/auth/sessions/'+encodeURIComponent(button.dataset.revokeSession),{method:'DELETE',headers:{'X-CSRF-Token':csrfToken,'Idempotency-Key':uuid()}});toast('设备已退出');await manageSessions();}catch(error){toast(error.message);}});const revokeOthers=body.querySelector('#revoke-other-sessions');if(revokeOthers)revokeOthers.onclick=async()=>{if(!confirm('确定退出除当前设备外的所有会话？'))return;try{await serverRequest('/api/auth/sessions/revoke-others',{method:'POST',headers:{'X-CSRF-Token':csrfToken,'Idempotency-Key':uuid()},body:JSON.stringify({reason:'User revoked all other sessions from device manager'})});toast('其他设备已全部退出');await manageSessions();}catch(error){toast(error.message);}};}catch(error){toast(error.message);}}
async function generateRecoveryCodes(){if(!confirm('生成新恢复码会立即作废旧恢复码。是否继续？'))return;const reason=prompt('请输入生成恢复码的审计理由');if(!reason)return;try{const result=await serverRequest('/api/auth/recovery-codes',{method:'POST',headers:{'X-CSRF-Token':csrfToken,'Idempotency-Key':uuid()},body:JSON.stringify({reason})}),body=securityDialog('一次性恢复码'),text=result.codes.join('\n');body.innerHTML='<p>这些恢复码只显示一次。请保存到离线密码管理器或打印后放入安全位置。</p><pre id="recovery-code-list" class="code"></pre><button id="download-recovery-codes" class="primary" type="button">下载恢复码</button>';body.querySelector('#recovery-code-list').textContent=text;body.querySelector('#download-recovery-codes').onclick=()=>downloadFile('reliacode-recovery-codes.txt',text+'\n','text/plain;charset=utf-8');}catch(error){toast(error.message);}}
...
const capabilities = () => {const values=sessionUser?.capabilities || ROLE_CAPABILITIES[currentRole()] || [];if(!domainApi)return new Set(values);const enabled=new Set(["platform:tenants:read","platform:tenants:write","tenant:manage","products:write","codes:write","codes:approve","objects:read","events:read","events:write:packing","events:write:unpacking","events:write:shipping","events:write:distributor_receiving","events:write:store_receiving","events:write:returning","events:write:selling","events:write:destroying","members:read","members:invite","members:manage"]);return new Set(values.filter(value=>enabled.has(value)));};
...
function updateChrome() {
  ...
  const userMenu=$("#user-menu"), popover=$("#user-menu-popover"), logout=$("#logout"),addPasskey=$("#add-passkey"),managePasskeyButton=$("#manage-passkeys"),manageSessionsButton=$("#manage-sessions"),recoveryCodeButton=$("#generate-recovery-codes");
  ...
  if(managePasskeyButton){managePasskeyButton.hidden=!domainApi;if(!managePasskeyButton.dataset.bound){managePasskeyButton.dataset.bound="1";managePasskeyButton.onclick=managePasskeys;}}
  if(manageSessionsButton){manageSessionsButton.hidden=!domainApi;if(!manageSessionsButton.dataset.bound){manageSessionsButton.dataset.bound="1";manageSessionsButton.onclick=manageSessions;}}
  if(recoveryCodeButton){recoveryCodeButton.hidden=!domainApi;if(!recoveryCodeButton.dataset.bound){recoveryCodeButton.dataset.bound="1";recoveryCodeButton.onclick=generateRecoveryCodes;}}
...
function renderCodes() {
  ...
  const gs1Panel=domainApi&&can("tenant:manage")?`<section class="panel"><div class="section-head"><div><h3>GS1 企业标识</h3><p>托盘 SSCC 使用租户所有者确认的 Company Prefix；首次分配后不可在线更换。</p></div></div><form id="gs1-settings-form" class="compact-form"><label>GS1 Company Prefix<input name="gs1CompanyPrefix" required inputmode="numeric" pattern="[0-9]{4,12}" minlength="4" maxlength="12" value="${escapeHtml(state.gs1Settings?.gs1CompanyPrefix||'')}" /></label><div><small>下一个 SSCC 引用号：${escapeHtml(state.gs1Settings?.ssccNextReference||'0')} · 总容量：${escapeHtml(state.gs1Settings?.ssccCapacity||'配置后计算')}</small></div><button class="secondary" type="submit">保存企业前缀</button></form></section>`:'';
...
async function addProduct(event) { event.preventDefault();const data=new FormData(event.currentTarget);const sku=String(data.get("sku")).trim(),gtin=String(data.get("gtin")).trim(),caseGtin=String(data.get("caseGtin")||"").trim();if(state.products.some((item)=>item.sku.toLowerCase()===sku.toLowerCase()))return toast("SKU 已存在");if(gtin&&!validGtin(gtin))return toast("GTIN 必须是有效的 8、12、13 或 14 位编码（含校验位）");if(caseGtin&&!validGtin(caseGtin))return toast("箱级 GTIN 必须包含有效校验位");if(gtin&&caseGtin&&gtin===caseGtin)return toast("单品和箱级贸易项目必须使用不同 GTIN");try{if(domainApi){await serverRequest('/api/v1/products',{method:'POST',headers:{'X-CSRF-Token':csrfToken,'Idempotency-Key':uuid()},body:JSON.stringify({name:String(data.get('name')).trim(),sku,gtin:gtin||undefined,caseGtin:caseGtin||undefined,auditReason:'管理员通过产品目录创建产品'})});await loadDomainState(sessionUser);}else{state.products.push({id:uuid(),name:String(data.get("name")).trim(),sku,gtin,createdAt:timestamp()});save();}renderAll();go("codes");toast("产品已保存");}catch(error){toast(error.message);} }
async function saveGs1Settings(event){event.preventDefault();const value=String(new FormData(event.currentTarget).get("gs1CompanyPrefix")).trim();try{await serverRequest('/api/v1/tenant/gs1-settings',{method:'POST',headers:{'X-CSRF-Token':csrfToken,'Idempotency-Key':uuid()},body:JSON.stringify({gs1CompanyPrefix:value,auditReason:'租户所有者确认 GS1 Company Prefix'})});await loadDomainState(sessionUser);renderAll();go("codes");toast("GS1 Company Prefix 已保存");}catch(error){toast(error.message);}}
...
async function generateBatch(event) { event.preventDefault();const data=new FormData(event.currentTarget);try{const quantity=Number(data.get("quantity")),level=String(data.get("level"));if(domainApi){await serverRequest('/api/v1/code-jobs',{method:'POST',headers:{'X-CSRF-Token':csrfToken,'Idempotency-Key':uuid()},body:JSON.stringify({productId:String(data.get('productId')),level,lot:String(data.get('lot')).trim()||undefined,quantity,serialRule:level==='PALLET'?'SEQUENTIAL':'RANDOM',auditReason:'管理员创建异步生码任务'})});await loadDomainState(sessionUser);}else createBatch({productId:String(data.get("productId")),level,lot:String(data.get("lot")).trim(),quantity});renderAll();go("codes");toast(domainApi?'生码任务已创建，等待审批':`已生成 ${quantity.toLocaleString()} 枚唯一可靠码`);}catch(error){toast(error.message);} }
...
const domainEventCapabilities={PACKING:"events:write:packing",UNPACKING:"events:write:unpacking",REPACKING:"events:write:packing",SHIPPING:"events:write:shipping",RECEIVING_DISTRIBUTOR:"events:write:distributor_receiving",RECEIVING_STORE:"events:write:store_receiving",RETURNING:"events:write:returning",SELLING:"events:write:selling",DESTROYING:"events:write:destroying"};
const domainEventLabels={PACKING:"装箱",UNPACKING:"拆箱",REPACKING:"换箱",SHIPPING:"发货",RECEIVING_DISTRIBUTOR:"渠道收货",RECEIVING_STORE:"门店收货",RETURNING:"退货",SELLING:"销售",DESTROYING:"销毁"};
function renderReceive() {
  if(domainApi)return renderDomainReceive();
...
function renderDomainReceive(){
  const allowed=Object.entries(domainEventCapabilities).filter(([,capability])=>can(capability)),documents=(state.documents||[]).filter(item=>["APPROVED","IN_PROGRESS"].includes(item.status)),devices=(state.devices||[]);
  $("#receive").innerHTML=`<div class="page-heading"><div><span class="section-kicker">在线作业</span><h2>服务端业务扫码</h2><p>核验不会写入；确认时由设备凭据、单据、租户权限和对象状态共同裁决。</p></div></div><div class="two-column"><section class="panel field-context-panel"><div class="section-head"><div><h3>受控作业上下文</h3><p>设备令牌仅保存在当前页面内存，刷新后必须重新输入。</p></div></div><label class="field-label">作业类型<select id="domain-event-type">${allowed.map(([type])=>`<option value="${type}">${domainEventLabels[type]}</option>`).join("")}</select></label><label class="field-label">业务单据<select id="domain-document-id"><option value="">请选择已批准单据</option>${documents.map(item=>`<option value="${escapeHtml(item.id)}">${escapeHtml(item.reference)} · ${escapeHtml(item.document_type)} · ${escapeHtml(item.status)}</option>`).join("")}</select></label><label class="field-label">注册设备<select id="domain-device-id"><option value="">请选择活动设备</option>${devices.map(item=>`<option value="${escapeHtml(item.id)}" ${operationalDeviceCredential.id===item.id?'selected':''}>${escapeHtml(item.name)} · ${escapeHtml(item.id)}</option>`).join("")}</select></label><label class="field-label">设备令牌<input id="domain-device-token" type="password" autocomplete="off" minlength="32" value="${escapeHtml(operationalDeviceCredential.token)}" placeholder="输入设备首次注册时交付的令牌" /></label></section><section class="panel"><div class="section-head"><div><h3>对象核验与确认</h3><p>首期只允许在线确认，网络错误不会创建本地待同步事件。</p></div></div><label class="field-label">可靠码<input id="field-code" autocomplete="off" placeholder="扫描或输入序列对象编码" /></label><button class="secondary camera-trigger" type="button" data-camera-target="field-code">${icon("scan")}摄像头扫码</button><label id="domain-parent-field" class="field-label" hidden>目标父包装码<input id="parent-code" autocomplete="off" placeholder="装箱或换箱时输入目标箱码" /></label><button id="validate-field" class="primary" type="button" ${allowed.length?'':'disabled'}>1. 在线核验</button><div id="field-result" class="scan-result empty">${emptyState(allowed.length?"等待扫码":"当前角色没有现场写权限",allowed.length?"核验通过后仍需明确确认。":"请联系租户管理员分配作业角色。")}</div></section></div>`;
  const eventSelect=$("#domain-event-type"),toggleParent=()=>{$("#domain-parent-field").hidden=!["PACKING","REPACKING"].includes(eventSelect?.value);};if(eventSelect){eventSelect.onchange=toggleParent;toggleParent();}
  $("#validate-field").onclick=validateDomainField;bindCameraButtons();
}
async function validateDomainField(){
  const eventType=$("#domain-event-type")?.value,documentId=$("#domain-document-id")?.value,deviceId=$("#domain-device-id")?.value,deviceToken=$("#domain-device-token")?.value||"",objectCode=scannedIdentity($("#field-code")?.value),parentObjectCode=scannedIdentity($("#parent-code")?.value||""),out=$("#field-result");
  if(!eventType||!documentId||!deviceId||deviceToken.length<32||!objectCode){out.innerHTML=`<div class="result-status danger-status">${icon("risk")}上下文不完整</div><p>请选择作业类型、业务单据和设备，输入设备令牌并扫描对象。</p>`;return;}
  if(["PACKING","REPACKING"].includes(eventType)&&!parentObjectCode){out.innerHTML=`<div class="result-status danger-status">${icon("risk")}缺少目标父包装</div>`;return;}
  try{
    const [objectResult,lineResult,parentResult]=await Promise.all([serverRequest('/api/v1/objects/'+encodeURIComponent(objectCode)),serverRequest('/api/v1/documents/'+encodeURIComponent(documentId)+'/objects'),parentObjectCode?serverRequest('/api/v1/objects/'+encodeURIComponent(parentObjectCode)):Promise.resolve(null)]);
    const line=(lineResult.items||[]).find(item=>item.object_id===objectResult.object.id);if(!line||!line.expected||line.line_role!=="ACTION"||line.fulfilled_event_id)throw new Error("该对象不是单据中尚未履约的动作行");
    if(parentResult){const parentLine=(lineResult.items||[]).find(item=>item.object_id===parentResult.object.id);if(!parentLine||!parentLine.expected)throw new Error("目标父包装不在同一业务单据中");}
    operationalDeviceCredential={id:deviceId,token:deviceToken};pendingFieldEvent={accountId:sessionUser.id,eventType,eventLabel:domainEventLabels[eventType],documentId,deviceId,deviceToken,code:objectResult.object.code,parentCode:parentResult?.object?.code||null};
    out.innerHTML=`<div class="result-status success-status">${icon("check")}在线核验通过</div><h3 class="code">${escapeHtml(objectResult.object.code)}</h3><p>${escapeHtml(objectResult.object.product_name)} · ${escapeHtml(objectResult.object.level)} · 当前状态 ${escapeHtml(objectResult.object.status)}</p><button id="confirm-field" class="primary" type="button">2. 确认${escapeHtml(domainEventLabels[eventType])}</button>`;$("#confirm-field").onclick=confirmDomainField;
  }catch(error){pendingFieldEvent=null;out.innerHTML=`<div class="result-status danger-status">${icon("risk")}不可确认</div><p>${escapeHtml(error.message)}</p>`;}
}
async function confirmDomainField(){
  const pending=pendingFieldEvent;if(!pending||pending.accountId!==sessionUser?.id)return toast("作业上下文已变化，请重新核验");const button=$("#confirm-field");if(button)button.disabled=true;
  try{const result=await serverRequest('/api/v1/trace-events',{method:'POST',headers:{'X-CSRF-Token':csrfToken,'Idempotency-Key':uuid(),'X-ReliaCode-Device-Id':pending.deviceId,'X-ReliaCode-Device-Token':pending.deviceToken},body:JSON.stringify({eventType:pending.eventType,objectCode:pending.code,documentId:pending.documentId,parentObjectCode:pending.parentCode||undefined,readPoint:'urn:reliacode:client:ignored',eventTime:timestamp(),metadata:{client:'scan-web'}})});pendingFieldEvent=null;await loadDomainState(sessionUser);renderAll();go('receive');toast(`${pending.eventLabel}已记录，影响 ${Number(result.affectedObjectCount||1).toLocaleString()} 个对象`);}catch(error){if(button)button.disabled=false;toast(error.message);}
}
...
async function loadDomainState(user){
  ...
  requests.push(available.has('tenant:manage')?serverRequest('/api/v1/tenant/gs1-settings'):Promise.resolve(null));
  const hasEventWork=[...available].some(value=>value==='events:read'||value.startsWith('events:write:'));
  requests.push(hasEventWork?serverRequest('/api/v1/documents'):Promise.resolve({items:[]}));
  requests.push(available.has('objects:read')?serverRequest('/api/v1/devices'):Promise.resolve({items:[]}));
  const [productsResult,jobsResult,applicationsResult,gs1Settings,documentsResult,devicesResult]=await Promise.all(requests);
  state={...cloneEmpty(),initialized:true,workspace:{id:user.tenantId||uuid(),brandName:user.organizationName||'ReliaCode 平台运营',createdAt:timestamp()},accounts:[],currentAccountId:user.id,
    products:(productsResult.items||[]).map(item=>({id:item.id,name:item.name,sku:item.sku,gtin:item.gtin||'',tradeItems:item.trade_items||{},status:item.status,createdAt:item.created_at})),
    codeBatches:(jobsResult.items||[]).map(item=>({id:item.id,productId:item.product_id,level:item.level,identifierScheme:item.identifier_scheme,lot:item.lot||'',quantity:Number(item.quantity),generatedCount:Number(item.generated_count||0),codes:[],firstCode:'—',lastCode:'—',status:item.status,createdAt:item.created_at})),documents:documentsResult.items||[],devices:(devicesResult.items||[]).filter(item=>item.status==='ACTIVE'),platformApplications:applicationsResult.items||[],gs1Settings,objects:{},events:[],campaigns:[],ledger:[],risks:[],agentRuns:[]};
...
function clearRecoveryUrl(){resetToken=null;verifyEmailToken=null;history.replaceState({},"",location.pathname);}
function renderAccountRecovery(){const overlay=$('#onboarding')||document.body.appendChild(Object.assign(document.createElement('div'),{id:'onboarding',className:'onboarding'}));overlay.hidden=false;const activeResetToken=resetToken,activeVerifyToken=verifyEmailToken;history.replaceState({},"",location.pathname);if(activeVerifyToken){overlay.innerHTML=`<section class="onboarding-card auth-card"><div class="onboarding-brand">${icon('logo')}<div><b>ReliaCode 可靠码</b><small>邮箱验证</small></div></div><h1>正在验证邮箱</h1><p id="recovery-status">请稍候…</p></section>`;serverRequest('/api/auth/email-verification/confirm',{method:'POST',body:JSON.stringify({token:activeVerifyToken})}).then(()=>{$('#recovery-status').innerHTML='邮箱已验证。<button id="recovery-login" class="secondary" type="button">返回登录</button>';$('#recovery-login').onclick=()=>{clearRecoveryUrl();showLogin();};}).catch(error=>{$('#recovery-status').textContent=error.message;});return;}overlay.innerHTML=`<form id="password-reset-confirm" class="onboarding-card auth-card"><div class="onboarding-brand">${icon('logo')}<div><b>ReliaCode 可靠码</b><small>账号恢复</small></div></div><h1>设置新密码</h1><p>重置成功后，所有已登录设备都会退出。</p><label>新密码<input name="password" type="password" required minlength="12" maxlength="200" autocomplete="new-password" /></label><label>确认新密码<input name="confirmPassword" type="password" required minlength="12" maxlength="200" autocomplete="new-password" /></label><button class="primary" type="submit">重置密码</button><p id="login-error" class="auth-error" role="alert"></p></form>`;$('#password-reset-confirm').onsubmit=async event=>{event.preventDefault();const data=new FormData(event.currentTarget),password=String(data.get('password'));if(password!==String(data.get('confirmPassword'))){$('#login-error').textContent='两次输入的密码不一致';return;}try{await serverRequest('/api/auth/password-reset/confirm',{method:'POST',body:JSON.stringify({token:activeResetToken,newPassword:password})});clearRecoveryUrl();showLogin();toast('密码已重置，请重新登录');}catch(error){$('#login-error').textContent=error.message;}};}
if(resetToken||verifyEmailToken)renderAccountRecovery();else if(pendingDeepLink)renderOnboarding();else if(!hostedApi)renderAll();
async function bootstrap() { if(resetToken||verifyEmailToken)return;if (!hostedApi) { if (state.initialized) ensureSessionAccount(account()); renderAll(); return; } try { const session=await serverRequest('/api/auth/session'); csrfToken=session.csrfToken; sessionUser=session.user;if(domainApi)await loadDomainState(session.user);else{await loadServerWorkspace({allowMigration:session.user?.id==='local-admin'});ensureSessionAccount(session.user);}serverReady=true;renderAll(); } catch (error) { showLogin(); } }
function showLogin() {
  ...
  const renderAuth=(mode='login')=>{
    ...
    if(mode==='recovery'){overlay.innerHTML=`<form id="password-reset-request" class="onboarding-card auth-card"><div class="onboarding-brand">${icon('logo')}<div><b>ReliaCode 可靠码</b><small>账号恢复</small></div></div><h1>恢复账号</h1><p>仅已验证邮箱可以接收恢复链接。无论账号是否存在，系统都会返回相同结果。</p><label>邮箱<input name="email" type="email" required maxlength="254" autocomplete="email" /></label><button class="primary" type="submit">发送恢复邮件</button><button id="return-login" class="secondary" type="button">返回登录</button><p id="login-error" class="auth-error" role="alert"></p></form>`;$('#return-login').onclick=()=>renderAuth('login');$('#password-reset-request').onsubmit=async event=>{event.preventDefault();const email=String(new FormData(event.currentTarget).get('email')).trim();try{await serverRequest('/api/auth/password-reset/request',{method:'POST',body:JSON.stringify({email})});event.currentTarget.innerHTML='<h1>请检查邮箱</h1><p>如果该邮箱已验证且账号有效，恢复邮件会在稍后送达。</p><button id="return-login" class="secondary" type="button">返回登录</button>';$('#return-login').onclick=()=>renderAuth('login');}catch(error){$('#login-error').textContent=error.message;}};return;}
    ...
    if($('#forgot-password'))$('#forgot-password').onclick=()=>renderAuth('recovery');
