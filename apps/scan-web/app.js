...
const EMPTY_STATE = Object.freeze({
  ...
  products: [], codeBatches: [], objects: {}, events: [], campaigns: [], ledger: [], risks: [], agentRuns: [], gs1Settings:null
});
const svgPaths = {
...
const capabilities = () => {const values=sessionUser?.capabilities || ROLE_CAPABILITIES[currentRole()] || [];if(!domainApi)return new Set(values);const enabled=new Set(["platform:tenants:read","platform:tenants:write","tenant:manage","products:write","codes:write","codes:approve","objects:read","events:read","members:read","members:invite","members:manage"]);return new Set(values.filter(value=>enabled.has(value)));};
...
async function loadDomainState(user){
  ...
  const [productsResult,jobsResult,applicationsResult,gs1Settings]=await Promise.all(requests);
